...
CREATE TABLE `OBSERVATORY` (
  `ID_OBSERVAT` integer primary key, -- Primary id
...
CREATE TABLE `PROCESSING_HISTORY` (
  ...
  `OBSERVAT_ID` integer, -- Pointing to ID_OBSERVAT in OBSERVATORY table
  `FRC_INFO_ID` integer, -- Pointing to ID_FRC_INFO in FRC_INFO table
  `DATE_OBS` text, -- Date of observation
  ...
  FOREIGN KEY(OBSERVAT_ID) REFERENCES OBSERVATORY(ID_OBSERVAT),
