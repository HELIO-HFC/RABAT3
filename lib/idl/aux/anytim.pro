...
ecs=ecs, vms=vms, stime=stime, truncate=truncate, tai=tai, error=error
...
if keyword_set(tai) then out='TAI'
if out eq 'MJD' then out = 'UTC_INT'

;
; Check for seconds and out to preserve sub-millisecond timing.
;
just_seconds = (out eq  'UTIME' or out eq 'SEC' or out eq 'SECONDS') and (typ eq 'DOU' or typ eq 'FLO')
...
case 1 of
    ...
    (siz(0) eq 1 and (siz(1) ne 2 and siz(1) ne 7)))):  begin
        ...
        ustr = utime2str( item(*), utbase=0.0d0)  ;ras, 8-sep-2005, prevent longword roundoff problem
...
out eq 'VMS' or out eq 'STIME' or out eq 'TAI' then begin
    ...
    if out eq 'TAI' then result = anytim2tai( result )
    if keyword_set(truncate) and not keyword_set(date) and $
    ( out eq 'ECS' or out eq 'VMS' or out eq 'STIME' ) then $
    result = strmid( result, 0, strpos(result(0),'.'))
    endif
